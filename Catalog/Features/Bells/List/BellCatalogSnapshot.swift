...
extension BellEntity {
    convenience init(record: BellRecord) {
        self.init(
            id: record.id,
            title: record.title,
            notes: record.notes,
            acquiredYear: record.acquiredYear,
            createdAt: record.createdAt,
            conditionRaw: record.condition.rawValue,
            acquisitionMethodRaw: record.acquisitionMethod.rawValue,
            materialRaw: record.details.material.rawValue,
            customMaterialName: record.details.customMaterialName,
            createdBy: record.createdBy
        )
        collection = CollectionEntity(
            id: record.item.collectionID,
            kindRaw: CollectionKind.bells.rawValue,
            title: "",
            notes: "",
            backgroundStyleRaw: CollectionBackgroundStyle.amber.rawValue
        )
        location = record.storageLocation.map {
            LocationEntity(
                id: $0.id,
        ...
        originPlace = record.originPlace.map {
            PlaceEntity(
                id: $0.id,
        ...
        mediaAssets = record.mediaAssets.map {
            MediaAssetEntity(
                id: $0.id,
                kindRaw: $0.kind.rawValue,
        ...
        tags = record.tags.enumerated().map { offset, tag in
            BellTagEntity(value: tag, sortOrder: offset)
        }
    }
}
